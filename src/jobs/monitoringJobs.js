...
async function checkAllUsersEmails() {
    try {
        ...
        // Process customers concurrently (limit 5)
        await limitConcurrency(customers, 5, async (customer) => {
...
async function checkCustomerEmails(authId) {
    try {
        ...
        // Process accounts concurrently (limit 3)
        await limitConcurrency(allAccounts, 3, async (account) => {
...
async function checkEmailsForAccount(account, authId) {
    try {
        ...
        if (newEmails.length > 0) {
            ...
            // Process emails concurrently (limit 5)
            await limitConcurrency(newEmails, 5, async (message) => {
...
async function processEmailMessage(message, accessToken, accountEmail, accountId, authId) {
    try {
        ...
        // Process attachments concurrently (limit 3)
        const results = await limitConcurrency(attachments, 3, async (attachment) => {
            try {
        ...
        allAttachmentsProcessed = results.every(r => r === true);
